...
| `make adaptive` | M6, added 2026-09-20. **MEASURED 2026-09-21: 3 h 44 min** (13,414 s, run `M6-AF-20260920T211148Z`, `SKIP_LLM=1`, 5 arms x 5 seeds; 134 CFD calls charged, 125 coarse-mesh cases run incl. 5 hold-outs, median case 156 s, 10.65 core-hours of CFD, 3 serial solvers side by side, on the 10-core machine shared with M7 on 4 workers and M5 - load average 6-77). `make adaptive-report` reproduces the report byte-for-byte (checked). Written before the run: **Needs OpenFOAM, gate G4 = PASS, and a DOE/M4 run on `cfd_surface_v2`** (it refuses otherwise; all three hold since 2026-09-21 - `scripts/check_study_preconditions.py` checks them without starting anything). Estimated **3.3–5.1 h**: 160–182 coarse-mesh CFD cases, 3 serial solvers side by side, from M3's measured 214 s median per case / 304 s mean per attempt; F0 evaluations add ~15 min and overlap. Makes ≤ 30 live `claude -p` calls unless `SKIP_LLM=1`. Run it in the background; if the source guard aborts it, `REUSE_CFD=<run>` reuses the finished CFD cases. |
